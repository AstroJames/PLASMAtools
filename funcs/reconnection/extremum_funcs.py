...
class Extremum():
    ...
    def __init__(self,
                 ...
                 n_steps_in_integration : int = 1000,
                 n_steps_in_limit_cycle : int = 400):#1500) -> None:
        ...
        #self.hessian = self.dvf.gradient_tensor(self.field)
        if self.debug:
    ...
    def __count_local_extrema(self):
        ...
        self.maximum_coords = peak_local_max(self.a_z,
                                             min_distance=self.min_distance)
    ...
    def __classify_critical_point(self):
        ...
        #     eigs = np.linalg.eigvals(self.hessian[:,:,coord[0],coord[1]])
        #     print(eigs)
        #     if (np.isclose(np.imag(eigs[0]),0.0,atol=0.1)) or (np.isclose(np.imag(eigs[1]),0.0,atol=0.1)):
        #         if self.debug:
        ...
        self.o_points = np.array(self.minimum_coords) * self.L / self.N_grid - self.L/2.0
        self.x_points = np.array(self.maximum_coords) * self.L / self.N_grid - self.L/2.0
...
if __name__ == "__main__":
    ...
    #data = h5py.File("/Users/beattijr/Documents/Research/2024/peta_scale_MHD/5040_slice_data/EXTREME_Turb_slice_xy_000633")
    
    #b_field = np.array([data["magy_slice_xy"],data["magy_slice_xy"]])
    ...
    ax.plot(data["y0"][data["type"]=="o_point"],
            data["x0"][data["type"]=="o_point"],
            c='r',marker='o',ls='None',markersize=4)
    ax.plot(data["y0"][data["type"]=="x_point"],
            data["x0"][data["type"]=="x_point"],
            c='y',marker='o',ls='None',markersize=4)
    # Create the ellipse patch
    for idx, row in data[data["type"]=="o_point"].iterrows():
        print((row["a_max"],row["a_min"]))
    ...
    ax.set_xlim(-0.5,0.5)
    ax.set_ylim(-0.1,0.1)
    plt.savefig("/Users/beattijr/Documents/Research/2024/X_and_O/plot/mag_energy_Feb.png")
    plt.close()